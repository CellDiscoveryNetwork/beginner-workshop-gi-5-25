...
Take a close look at the results. When we modulate nfeatures or nPCs, we don't directly see changes in the number of clusters found. But, like we saw in the previous sessions, modulating the k.param and the clustering resolution have outsized effects on the number of clusters found.
...
First let's check how much the HVG change between both resolutions
```{r}
tse <- tse %>%
    NormalizeData() %>% 
    FindVariableFeatures(
...
Looking into what these genes are
```{r}
head(setdiff(VariableFeatures(tse_sub),  hvg_full), n = 100)
head(intersect(VariableFeatures(tse_sub),  hvg_full), n = 100)
```

We see that within the intersecting genes we see a lot of immunoglobulin-related genes. Let's adress that and check if we need to clean up this level of annotation.
...
Let's look at the different cluster resolutions
```{r}
...
Now let's check where the HVG fall
```{r fig.width=9, fig.height=6}
VlnPlot(
    tse_sub,
    features = c("JCHAIN", "IGKC", "MZB1", "IGHG1", "CD3E", "MKI67", "S100A8", "HBB"),
    group.by = "RNA_snn_res.0.15",
    pt.size = 0)
```

Clearly cluster 7 is driving a lot of the variability in this susbset which we don't want since it seems to encompass proliferating cells along with cluster 6. For the purpose of this exercise we are going to remove them so we clearly focus on the T cells. Moreover, cluster 3 seems to be myeloid cell doublets.
...
Re-process the object to obtain T cell specific HVGs
```{r}
tse_sub <- tse_sub %>%
    NormalizeData() %>%
    FindVariableFeatures(
...
head(setdiff(VariableFeatures(tse_sub),  hvg_full), n = 100)
head(intersect(VariableFeatures(tse_sub),  hvg_full), n = 100)
head(VariableFeatures(tse_sub), n = 100)
```

Look at the updated embedding
Let's look at the different cluster resolutions
```{r}
# Visualize these clusters
DimPlot(
...
Then let's process our T cell subset object with the HVG obtained from the whole dataset - `hvg_full`:
```{r}
tse_full <- tse_sub %>%
    ScaleData(
        verbose = FALSE,
...
-   PC1 contains a lot of ribosomal genes in the positive arm and cytotoxic in the negative
-   PC2 contains genes related to activated T cells like CD69
...
Clusters 6 and 7 seem to be proliferating cells. 6 appeared to have a mix of T and Myeloid cells and 7 are B cells. Moreover, cluster 3 is simultaneously expressing Myeloid and T cell genes which could be an indication that these are doublets. Let's double check these cells are in fact doublets:
